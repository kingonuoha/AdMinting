...
class BrandsOnboarding extends Component
{
    ...
    {
        return $this->dispatchBrowserEvent('social:Facebook');
    }
    ...
    public function savePhoneNumber()
    {
        ...
        // an error happens where the phone number is not an array, so, we check if phone number list is emppty, then we convert the string into an array for input of numbers
        if(empty($this->phone_numbers) && !is_iterable($this->phone_numbers)){
            $this->phone_numbers = [];
        }

        // dd($this->phone_number, $this->phone_numbers);
        $number_exists = in_array($this->phone_number,$this->phone_numbers);
        if ($number_exists) {
